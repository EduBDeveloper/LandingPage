...
$(document).ready(function() {
    ...
    $(document).prop('title', 'Resumen de Órdenes - EduBWeb');
});
...
$(document).ready(function() {
    $.post(
        "../../modules/ordenes/consultar-orden.php", { FILTER: "ALL", ESTADO: "ALL" },
        function(data) {
...
$('select[name="ordenes_tipo"]').on("change", function() {
    ...
    if (ORD_TIPO != "") {
        ...
        $.post(
            "../../modules/ordenes/consultar-orden.php", { FILTER: "ALL", ESTADO: "ALL", TIPO_ORDEN: ORD_TIPO },
            function(data) {
                tbl_ordenes.clear().draw();
                data_orden = JSON.parse(data);
                for (i = 0; i < data_orden.length; i++) {
...
$("#btn-buscar").click(function() {
    ...
    if (orden_nroo != "" || orden_prov != "" || (orden_fini != "" && orden_ffin != "")) {
        ...
        $.post(
            "../../modules/ordenes/filtrar-ordenes.php", { orden_nroo: orden_nroo, orden_prov: orden_prov, orden_fini: orden_fini, orden_ffin: orden_ffin },
            function(data) {
                tbl_ordenes.clear().draw();
                data_orden = JSON.parse(data);
                for (i = 0; i < data_orden.length; i++) {
    ...
    callback: function(key, options) {
        ...
        if (tbl_data.length > 0) {
            ...
            switch (accion) {
                ...
                case "delete":
                    $.post(
                        "../../modules/ordenes/anular-orden.php", { ID_ORDEN: row_id },
                        function(data) {
                            if (data == true) {
                                TIPO_ORDEN = row_tipo == 1 ? "COMPRA" : "SERVICIO";
                                $.post(
                                    "../../modules/ordenes/consultar-orden.php", { FILTER: "ALL", ESTADO: "ALL", TIPO_ORDEN: TIPO_ORDEN },
                                    function(data) {
                                        tbl_ordenes.clear().draw();
                                        data_orden = JSON.parse(data);
                                        for (i = 0; i < data_orden.length; i++) {
