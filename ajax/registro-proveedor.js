...
$(document).ready(function() {
    ...
    $(document).prop('title', 'Proveedores - EduBWeb');
});
...
// Validación para que el campo de país no acepte números
$('input[name="proveedor_pais"]').on('keydown keypress paste', function(e) {
    if (e.type === 'paste') {
...
// Validación para que el campo de RUC no acepte letras
$('input[name="proveedor_numero"]').on('keydown keypress paste', function(e) {
    if (e.type === 'paste') {
        var clipboardData = e.originalEvent.clipboardData || window.clipboardData;
        var pastedData = clipboardData.getData('Text');
        if (/[^\d]/.test(pastedData)) {
...
// Validación para que el campo de teléfono solo acepte números
$('input[name="proveedor_conttelef_1"], input[name="proveedor_conttelef_2"]').on('keydown keypress paste', function(e) {
    if (e.type === 'paste') {
        var clipboardData = e.originalEvent.clipboardData || window.clipboardData;
        var pastedData = clipboardData.getData('Text');
        if (/[^\d]/.test(pastedData)) {
