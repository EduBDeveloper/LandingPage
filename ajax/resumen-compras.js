...
$(document).ready(function() {
    ...
    $(document).prop('title', 'Resumen de Compras - EduBWeb');
});
...
$("#btn-buscar").click(function() {
    ...
    $.post(
        "../../modules/compras/filtrar-compras.php", { orden_nroo: orden_nroo, orden_prov: orden_prov, orden_fini: orden_fini, orden_ffin: orden_ffin },
        function(data) {
            tbl_ordenes.clear().draw();
            data_orden = JSON.parse(data);
            for (i = 0; i < data_orden.length; i++) {
    ...
    callback: function(key, options) {
        ...
        if (tbl_data.length > 0) {
            ...
            switch (accion) {
                ...
                case "delete":
                    $.post(
                        "../../modules/compras/anular-compra.php", { ID_ORDEN: row_id },
                        function(data) {
                            if (data == true) {
                                listarCompras();
...
function listarCompras() {
    ...
    $.post(
        "../../modules/compras/filtrar-compras.php", { orden_nroo: "", orden_prov: "", orden_fini: "", orden_ffin: "" },
        function(data) {
            tbl_ordenes.clear().draw();
            data_orden = JSON.parse(data);
            for (i = 0; i < data_orden.length; i++) {
