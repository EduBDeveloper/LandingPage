...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    // Validaciones
    if (!ctype_digit($p_provnum)) {
        die("El RUC solo debe contener números.");
    }

    if (preg_match('/[0-9]/', $p_pais)) {
        die("El país no debe contener números.");
    }

    if (!ctype_digit($p_contelf1) || !ctype_digit($p_contelf2)) {
