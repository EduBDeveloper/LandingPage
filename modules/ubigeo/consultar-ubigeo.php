...
switch ($MODO_UBIGEO) {
    case 'DEPARTMENTS':
        $sqlStatement = $pdo->prepare("SELECT * FROM ubigeo_py_departments");
    ...
    case 'PROVINCES':
        ...
        $sqlStatement = $pdo->prepare("SELECT * FROM ubigeo_py_ciudad WHERE department_id=:IDDEP");
        ...
        if ($rowsNumber > 0) {
            array_push($DATA, ["id"=>"","text"=>"Seleccione una ciudad"]);
            while ($LST = $sqlStatement->fetch()) {
                $ID_PROV = $LST["province_id"];
                $NOM_PROV = $LST["name"];
                $ROW = [
                    "id" => $ID_PROV,
                    "text" => $NOM_PROV
